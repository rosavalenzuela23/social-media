...
import UserMapper from "@profiles/infraestructure/persistance/mappers/user.mapper.js";
...
export default class MongoRepository implements IProfileRepository {
  ...
  async getUserByUuid(uuid: string): Promise<Profile | null> {
    const profile = await this.dbContainer.getRepository(UserEntity).findOne({
      where: {
        uuid,
      },
    });
